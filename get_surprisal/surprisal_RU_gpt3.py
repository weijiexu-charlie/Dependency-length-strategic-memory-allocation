...
CORPUS = 'data/parsed_corpora/Russian/SUD_RU_SynTagRus.csv'
SEED = 100
OUTPUT_FILE = 'data/surprisal/SUD/Russian/SUD_RU_SynTagRus_logp.csv'
OUTPUT_FILE_TEST = 'data/surprisal/SUD/Russian/SUD_RU_SynTagRus_logp_test.csv'
OUTPUT_FILE_SMALL = 'data/surprisal/SUD/Russian/SUD_RU_SynTagRus_logp_small.csv'


pd.set_option('mode.chained_assignment', None)


def get_doc_dfs(corpus, max_wnum=250, min_wnum=50):
	'''
	Get a list of document texts from corpus for LM.
	Less than 300 and more than 50 words fed into the model.
...
def add_text_logp(doc_df):
	...
	'''
	encode, decode = 'latin-1', 'utf-8'
	tokens_corpus = doc_df.word_form.astype(str).tolist()
	text = utils.get_text_4lm(doc_df)
	model_output = surprisalGPT3.get_text_surprisal(text, encode, decode)
